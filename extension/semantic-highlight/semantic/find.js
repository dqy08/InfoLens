...
(() => {
  ...
  if (!globalThis.IL_statusFeedback) {
    throw new Error('IL_statusFeedback missing — inject statusFeedback.js first');
  }
  ...
  */
  ...
  function applyScopeDivider(atCp) {
    ...
    let parent;
    if (geo().isWindowScrollRoot(scrollRoot)) {
  ...
  function showFindStatus(label, detail, opts) {
    ...
    if (tone === 'error' && feedbackBtn instanceof HTMLButtonElement) {
      globalThis.IL_statusFeedback.resetButton(feedbackBtn);
    }
    list.appendChild(el);
  ...
  function progressAxisYRange() {
    return geo().axisYRange(doc.getRoot(), doc.findScrollRoot());
  }

  function viewportYAtRatio(ratio) {
    const scrollRoot = doc.findScrollRoot();
    if (geo().isWindowScrollRoot(scrollRoot)) return window.innerHeight * ratio;
  ...
  function chunkIndexAtFocusY(chunks, scrollRoot, toPaint = (c) => c) {
    const focusY = geo().contentYFromClientY(viewportFocusY(), scrollRoot);
    let landed = -1;
  ...
  function updateViewportFollowScrollBinding() {
    ...
    const needsTarget = !!(root && !geo().isWindowScrollRoot(root));
    if (!needsTarget) {
      unbindViewportFollowScrollTarget();
      return;
    }
  ...
  function scrollToContentY(contentY, highlightChunks) {
    ...
    const scrollRoot = doc.findScrollRoot();
    const focus = geo().scrollTopAtContentY(
      contentY,
      scrollRoot,
    ...
    if (highlightChunks && highlightChunks.length) {
      ...
      if (startY != null) {
        const clamp = geo().scrollTopAtContentY(
          startY,
          scrollRoot,
  ...
  function enqueueFollow(item) {
    if (item.reveal) {
      ...
      if (nextRect) {
        scrollToContentY(geo().contentYFromClientY(nextRect.top, scrollRoot));
      } else {
