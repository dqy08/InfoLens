...
const extDir = path.resolve(sourceDir, '..', 'dist', 'semantic-highlight');
...
function startFixtureServer() {
  const html = fs.readFileSync(path.join(sourceDir, 'e2e/fixtures/article.html'));
...
test('webpage find: inject, search, highlight', async () => {
  if (!fs.existsSync(path.join(extDir, 'config.js'))) {
    throw new Error('missing extension/dist/semantic-highlight/config.js — run the extension build first');
  }
  ...
  try {
    ...
    await expect.poll(() => hits.relevance, { timeout: 20_000 }).toBeGreaterThan(0);
    await expect.poll(() => hits.keywords, { timeout: 20_000 }).toBeGreaterThan(0);
