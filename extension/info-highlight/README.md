...
右键「Analyze this page」等同点工具栏；「Always analyze example.com」→ 当场申请该 hostname 的 host 权限（子域各算各的），之后该站前台标签在加载中即可自动分析（不必等转圈结束）；`complete` 与本轮分析收尾时各核对一次正文，变了就重跑（重复段走缓存）。
