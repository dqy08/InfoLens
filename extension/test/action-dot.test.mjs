...
test('Info Highlight 本版：升级后 intensity / one-tone / auto-sites / cloud model 为新；工具栏与选项解耦', async () => {
  ...
  assert.ok(!seed.includes('ih_cloud_model'));
  assert.ok(!seed.includes('ih_article_only'));
  ...
  assert.equal(
    ...
    'ih_article_only,ih_auto_sites,ih_cloud_model,ih_highlight_options_page,ih_max_highlight_alpha,ih_two_tier,ih_word_merge',
  );
});
